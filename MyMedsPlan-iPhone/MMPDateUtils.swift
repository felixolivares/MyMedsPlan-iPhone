...
class MMPDateUtils {
    ...
    public static func remainingTimeForNextIntake(hours:Int16) -> String{
        let date = Calendar.current.date(byAdding: .hour, value: Int(hours), to: Date())
        let now = Date()
        let newDate = date!.offset(from: now)
        return newDate
    }
    
    public static func remainingTimeForNextIntake(date:Date) -> String{
        let now = Date()
        let newDate = date.offset(from: now)
        return newDate
    }
